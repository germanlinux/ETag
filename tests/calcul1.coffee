...
myetag = new ETag
myetag.createETag({'url': '/foo/zombi.jpg', 'resource': 'tests/zombi.jpg'})
...
puts(chaine)
myetag = new ETag({'url': '/foo/zombi.jpg', 'resource': 'tests/zombiland'})
...
puts(chaine)
puts ETag.Calcul({'url': '/foo/zombi.jpg', 'resource': 'tests/zombi.jpg'})
try
        assert.equal(ETag.Calcul({'url': '/foo/zombi.jpg', 'resource': 'tests/zombi.jpg'}),'"3813434-33774-1331909155000"')
        puts(chaine + '  : ok')
        checkDone()
catch error
        puts("ERROR: " + error.expected + " (expected) <==> (actuel) " + error.actual)
